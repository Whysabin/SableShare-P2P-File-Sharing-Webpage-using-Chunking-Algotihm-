...
// Send file
const BYTES_PER_CHUNK = 32 * 1024; // 16KB
...
async function readNextChunk() {
  while (BYTES_PER_CHUNK * currentChunk < file.size) {
    ...
    console.log(`Sending chunk ${currentChunk + 1} of ${Math.ceil(file.size / BYTES_PER_CHUNK)} for file ${file.name}`);
...
function sendMessageQueued() {
  ...
  while (message) {
    ...
    try {
      ...
      console.log(`Sent chunk of size ${message.byteLength} bytes`);
...
function onReceiveMessageCallback(event) {
  ...
  if (!downloadInProgress) {
    ...
    progressDownload(event.data);
    console.log(`Received chunk of size ${event.data.byteLength} bytes`);
  }
}
...
function endDownload() {
  ...
  if (incomingFiles.length === 0) {
      ...
      receiveprogressbar.value = 0;
      clearInterval(statsUpdateInterval);
      showReceivedPopup(); // Show the success popup
  } else {
      startDownload();
  }
}

// Function to show the "File Received Successfully" popup
function showReceivedPopup() {
  const popup = document.getElementById("file-received-popup");
  popup.style.display = "block";
}

// Close button for the popup
document.getElementById("close-popup").addEventListener("click", () => {
  document.getElementById("file-received-popup").style.display = "none";
});
